...
echo [Step 1] Checking if MySQL is running on port 3307...
netstat -an | findstr ":3307" >nul 2>&1
if %errorlevel% equ 0 (
    echo [OK] MySQL is listening on port 3307
    netstat -an | findstr ":3307" | findstr "LISTENING"
) else (
    echo [ERROR] MySQL is NOT listening on port 3307!
    echo MySQL server is not running or not configured for port 3307.
    echo.
    echo Please:
    echo 1. Start MySQL service
    echo 2. Configure MySQL to use port 3307
...
echo Server=localhost;Port=3307;Database=POS_NEXA_ERP;Uid=root;Pwd=Shivaanica;AllowPublicKeyRetrieval=True;
echo.
...
findstr /C:"Port=3307" "FinalPOS\App.config" >nul 2>&1
if %errorlevel% neq 0 (
    echo [ERROR] Port is NOT 3307 in App.config!
    echo The connection string should have Port=3307
)
...
echo ========================================
echo.
echo If MySQL is running on port 3307 but connection fails:
echo 1. Check App.config has Pwd=Shivaanica (not Pwd=;)
echo 2. Check App.config has Port=3307
